(function() {
	...
	var viewEl = document.querySelector('.view'),
		gridEl = viewEl.querySelector('.gridADIDAS'),
	...
	function CompareBasket() {
		...
		this.closeCompareCtrl.addEventListener('click', function() {
			...
			showProduct.add(self.el, 'compare-basket--active');
			// animate..
			showProduct.remove(viewEl, 'view--compare');
		});
	}
	...
	CompareBasket.prototype.add = function(item) {
		...
		showProduct.add(item, 'product--selected');
		...
		this.totalItems++;
		if( this.isFull() ) {
			showProduct.add(this.el, 'compare-basket--full');
		}

		showProduct.add(this.el, 'compare-basket--active');
	};
	...
	CompareBasket.prototype._createItemPreview = function(item) {
		...
		removeCtrl.innerHTML = '<i class="fas fa-remove"></i><span class="action__text action__text--invisible">Remove product</span>';
	...
	CompareBasket.prototype.remove = function(item) {
		showProduct.remove(this.el, 'compare-basket--full');
		showProduct.remove(item, 'product--selected');
		...
		if( this.totalItems === 0 ) {
			showProduct.remove(this.el, 'compare-basket--active');
		}
	...
	CompareBasket.prototype._compareItems = function() {
		...
		setTimeout(function() {
			// toggle compare basket
			showProduct.remove(self.el, 'compare-basket--active');
			// animate..
			showProduct.add(viewEl, 'view--compare');
		}, 25);
	};
